...
List forwardbackwardx(const arma::mat& transition, NumericVector emissionArray,
  ...
  for (unsigned int k = 0; k < obs.n_slices; k++) {
    arma::mat atmp(emission.n_rows, obs.n_cols);
    arma::vec stmp(obs.n_cols);
  uvForward(sp_trans.t(), emission, initk.col(k), obs.slice(k), atmp, stmp);
alpha.slice(k) = atmp;
scales.col(k) = stmp;
  }
  if(!scales.is_finite()) {
  ...
  if (forwardonly) {
    ...
    for (unsigned int k = 0; k < obs.n_slices; k++) {
      arma::mat btmp(emission.n_rows, obs.n_cols);
      arma::vec stmp(obs.n_cols);
      stmp = scales.col(k);
      uvBackward(sp_trans, emission, obs.slice(k), btmp, stmp);
      beta.slice(k) = btmp;
    }
    if(!beta.is_finite()) {
