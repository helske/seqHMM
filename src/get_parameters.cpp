...
// [[Rcpp::export]]
arma::mat get_pi_qs(const arma::cube& gamma, const arma::mat& X, 
                    const arma::vec& probs) {
  unsigned int S = gamma.n_rows;
  ...
  for (unsigned int i = 0; i < N; i++) {
    for (unsigned int l = 0; l < L; l++) {
      pi.col(l) = get_pi(gamma.slice(l), X.col(i));
    }
...
arma::mat get_B_qs(const arma::field<arma::cube>& gamma, 
  ...
  for (unsigned int i = 0; i < N; i++) {
    for (unsigned int l = 0; l < L; l++) {
      B.col(l) = arma::vectorise(get_B(gamma(l), X.slice(i), tv, false));
    }
...
// [[Rcpp::export]]
arma::mat get_omega_qs(const arma::cube& gamma, const arma::mat& X,
                       const arma::vec& probs) {
  unsigned int D = gamma.n_rows;
  unsigned int L = gamma.n_slices;
  unsigned int N = X.n_cols;
  unsigned int P = probs.n_elem;
  arma::mat omega(D, L);
  ...
  for (unsigned int i = 0; i < N; i++) {
    for (unsigned int l = 0; l < L; l++) {
      omega.col(l) = get_omega(gamma.slice(l), X.col(i));
    }
...
// probs is a vector of length P
// [[Rcpp::export]]
arma::mat get_pi_ame(const arma::cube& gamma, 
  ...
  for (unsigned int l = 0; l < L; l++) {
    for (unsigned int i = 0; i < N; i++) {
      pi.col(l) += invN * (
        get_pi(gamma.slice(l), X1.col(i)) - get_pi(gamma.slice(l), X2.col(i))
      );
    }
  }
  qs.col(0) = arma::mean(pi, 1);
  qs.cols(1, P) = arma::quantile(pi, probs, 1);
  return qs;
}
// gamma is L field of S x K x S cubes
// X is K x T x N cube (covariates, time points, sequences)
...
arma::mat get_A_ame(const arma::field<arma::cube>& gamma, 
  ...
  qs.col(0) = arma::mean(A, 1);
  qs.cols(1, P) = arma::quantile(A, probs, 1);
  return qs;
}
...
arma::mat get_B_ame(const arma::field<arma::cube>& gamma, 
  ...
  qs.col(0) = arma::mean(B, 1);
  qs.cols(1, P) = arma::quantile(B, probs, 1);
  return qs;
}
// gamma is D x K x L (cluster, covariates, sample)
// X a K x N matrix
// [[Rcpp::export]]
arma::mat get_omega_ame(const arma::cube& gamma, 
                     const arma::mat& X1, const arma::mat& X2, 
                     const arma::vec& probs) {
  unsigned int D = gamma.n_rows;
  unsigned int L = gamma.n_slices;
  ...
  for (unsigned int l = 0; l < L; l++) {
    for (unsigned int i = 0; i < N; i++) {
      omega.col(l) += invN * (
        get_omega(gamma.slice(l), X1.col(i)) - get_omega(gamma.slice(l), X2.col(i))
      );
    }
  }
  qs.col(0) = arma::mean(omega, 1);
  qs.cols(1, P) = arma::quantile(omega, probs, 1);
  return qs;
}