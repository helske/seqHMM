...
double nhmm::objective_pi(const arma::vec& x, arma::vec& grad) {
  
  mstep_iter++;
  double value = 0;
  eta_pi = arma::mat(x.memptr(), S - 1, X_pi.n_rows);
...
double nhmm::objective_A(const arma::vec& x, arma::vec& grad) {
...
double nhmm::objective_B(const arma::vec& x, arma::vec& grad) {
  mstep_iter++;
  double value = 0;
...
void nhmm::mstep_pi(const arma::uword print_level) {
  mstep_return_code = 0;
...
void nhmm::mstep_A(const arma::uword print_level) {
  mstep_return_code = 0;
...
void nhmm::mstep_B(const arma::uword print_level) {
  mstep_return_code = 0;
  // use closed form solution
  ...
  for (arma::uword c = 0; c < C; ++c) {
    ...
    for (arma::uword s = 0; s < S; ++s) {
      ...
      // why? 
      eta_B(c).slice(s) = arma::mat(x.memptr(), M(c) - 1, X_B(c).n_rows);
    }
  }
}

// [[Rcpp::export]]
Rcpp::List EM_LBFGS_nhmm(
...
Rcpp::List EM_LBFGS_fanhmm(
    ...
    const arma::cube& eta_A,
    const arma::field<arma::cube>& eta_B,
    const arma::field<arma::cube>& W, const arma::field<arma::vec>& delta,
    const double lambda,
    const arma::uword maxeval, const double ftol_abs, const double ftol_rel, 
    const double xtol_abs, const double xtol_rel, const arma::uword print_level,
    const arma::uword maxeval_m, const double ftol_abs_m, const double ftol_rel_m, 
  ...
  fanhmm model(
      obs, Ti, M, X_pi, X_A, X_B, icpt_only_pi, icpt_only_A, icpt_only_B, 
      iv_A, iv_B, tv_A, tv_B, eta_pi, eta_A, eta_B, W, delta, lambda
  );
  return model.EM(
    maxeval, ftol_abs, ftol_rel, xtol_abs, xtol_rel, print_level, 
    maxeval_m, ftol_abs_m, ftol_rel_m, xtol_abs_m,  xtol_rel_m, print_level_m,
