...
#pragma omp parallel for if(obs.n_rows >= threads) schedule(static) num_threads(threads) \
  default(none) shared(q, alpha, beta, ll, gradmat, nSymbols, ANZ, BNZ, INZ,         \
            ...
            if (ind.n_elem > 0) {
              ...
              for (int j = 0; j < numberOfStates(jj); j++) {
                for (unsigned int t = 0; t < (obs.n_cols - 1); t++) {
                  ...
                  gradArow(j) += exp(alpha(cumsumstate(jj)-numberOfStates(jj)+i,t,k) + tmp + 
        ...
        for (int i = 0; i < numberOfStates.size(); i++) {
          ...
          if (ind.n_elem > 0) {
            ...
            for (int j = 0; j < numberOfStates(i); j++) {
              ...
              gradIrow(j) += exp(tmp + beta(cumsumstate(i)-numberOfStates(i)+j,0,k)-ll(k)+weights(i,k));          
      ...
      for (int jj = 1; jj < numberOfStates.size(); jj++) {
        for (int j = 0; j < emission.n_rows; j++) {
          ...
          if ((j >= (cumsumstate(jj) - numberOfStates(jj))) & (j < cumsumstate(jj))) {
            gradmat.col(k).subvec(countgrad + q * (jj - 1), countgrad + q * jj - 1) +=
              exp(tmp+beta(j,0,k)-ll(k)+initk(j,k))*X.row(k).t()*(1.0 - exp(weights(jj,k))); 
          } else {
            gradmat.col(k).subvec(countgrad + q * (jj - 1), countgrad + q * jj - 1) -= 
              exp(tmp+beta(j,0,k)-ll(k)+initk(j,k))*X.row(k).t()*exp(weights(jj,k));
          }
        }
        
      }
    }
    return List::create(Named("objective") = -sum(ll), 
