...
double nhmm_base::objective_pi(const arma::vec& x, arma::vec& grad) {
  ...
  for (arma::uword i = 0; i < N; i++) {
    ...
    double val = arma::as_scalar(E_Pi.col(i).t() * tmp - sum_e * logSumExp(tmp));
    if (!std::isfinite(val)) {
      if (!grad.is_empty()) {
  ...
  return value + 0.5 * penalty * arma::dot(x, x);
}
...
Rcpp::List EM_LBFGS_nhmm_singlechannel(
  ...
  nhmm_sc model(
      ...
      iv_pi, iv_A, iv_B, tv_A, tv_B, obs, eta_pi, eta_A, eta_B, penalty
  );
  ...
  double penalty_term = 0.5 * penalty * arma::dot(previous_pars, previous_pars);
  ll -= penalty_term;
  ...
  while (relative_change > ftol_rel && absolute_change > ftol_abs && 
    ...
    penalty_term = 0.5 * penalty * arma::dot(current_pars, current_pars);
    ll_new -= penalty_term;
  ...
  return Rcpp::List::create(
    ...
    Rcpp::Named("penalty") = penalty_term,
