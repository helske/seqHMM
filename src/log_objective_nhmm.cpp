...
Rcpp::List log_objective_mnhmm_singlechannel(
  ...
  for (unsigned int i = 0; i < N; i++) {
    ...
    for (unsigned int d = 0; d < D; d++) {
      log_Pi(d) = get_pi(gamma_pi_raw(d), X_i.col(i), 1);
      log_A(d) = get_A(gamma_A_raw(d), X_s.slice(i), 1);
      log_B(d) = get_B(gamma_B_raw(d), X_o.slice(i), 1, 1);
      for (unsigned int t = 0; t < T; t++) {
        for (unsigned int s = 0; s < S; s++) {
          log_py(s, t, d) = log_B(d).at(s, obs(t, i), t);
        }
      }
      log_alpha.slice(d) = univariate_forward_nhmm(log_Pi(d), log_A(d), log_py.slice(d));
    ...
    // d loglik / d pi
    for (unsigned int d = 0; d < D; d++) {
      gradvec_S = exp(log_omega(d) + log_py.slice(d).col(0) + log_beta.slice(d).col(0) - loglik(i));
      // d pi / d gamma_pi
      Pi = exp(log_Pi(d));
      gradmat_S = -Pi * Pi.t();
      gradmat_S.diag() += Pi;
      ...
      for (unsigned int t = 0; t < (T - 1); t++) {
        ...
        for (unsigned int s = 0; s < S; s++) {
          // d loglik / d a_s
          gradvec_S = exp(log_omega(d) + log_alpha(s, t, d) + log_py.slice(d).col(t + 1) +
            log_beta.slice(d).col(t + 1) - loglik(i));
          // d a_s / d gamma_A
          gradmat_S = -A.row(s).t() * A.row(s);
          gradmat_S.diag() += A.row(s);
          grad_A(d).slice(s) += gradmat_S.rows(1, S - 1) * gradvec_S * X_s.slice(i).col(t).t();
        }
      }
      for (unsigned int s = 0; s < S; s++) {
...
Rcpp::List log_objective_mnhmm_multichannel(
  ...
  arma::field<arma::mat> grad_pi(D);
  arma::field<arma::cube> grad_A(D);
  arma::field<arma::cube> grad_B(D, C);
  for (unsigned int d = 0; d < D; d++) {
    grad_pi(d) = arma::mat(S - 1, X_i.n_rows, arma::fill::zeros);
    grad_A(d) = arma::cube(S - 1, X_s.n_rows, S, arma::fill::zeros);
    for (unsigned int c = 0; c < C; c++) {
    ...
    for (unsigned int d = 0; d < D; d++) {
      ...
      for (unsigned int t = 0; t < T; t++) {
        for (unsigned int s = 0; s < S; s++) {
          log_py(s, t, d) = 0;
          for (unsigned int c = 0; c < C; c++) {
            log_py(s, t, d) += log_B(d * C + c).at(s, obs(c, t, i), t);
          }
        }
      }
      log_alpha.slice(d) = univariate_forward_nhmm(log_Pi(d), log_A(d), log_py.slice(d));
