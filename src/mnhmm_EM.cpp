...
//       Pi = softmax(tmp);
//     }
//     double sum_e = sum(E_Pi(current_mixture).col(i));
//     
//     double val = arma::as_scalar(E_Pi(current_mixture).col(i).t() * tmp - sum_e * logSumExp(tmp));
//     if (!std::isfinite(val)) {
//       if (!grad.is_empty()) {
...
//     if (!grad.is_empty()) {
//       grad -= Qt * (E_Pi(current_mixture).col(i) - sum_e * Pi(current_mixture)) * X_pi.col(i).t();
//       if (!std::isfinite(arma::accu(grad))) {
//         grad.fill(std::numeric_limits<double>::max());
//         return std::numeric_limits<double>::max();
//       }
//     }
...
//   };
//   
//   arma::vec x_pi = arma::vectorise(eta_pi(current_mixture));
...
//   }
//   eta_pi(current_mixture) = arma::mat(x_pi.memptr(), S - 1, K_pi);
...
//       
//       double val = arma::as_scalar(E_A(current_s).slice(t).col(i).t() * tmp - sum_e * logSumExp(tmp));
//       if (!std::isfinite(val)) {
//         if (!grad.is_empty()) {
//           grad.fill(std::numeric_limits<double>::max());
//         }
...
//       if (!grad.is_empty()) {
//         grad -= arma::vectorise(
//           Qt * (E_A(current_s).slice(t).col(i) - sum_e * A1) * X_A.slice(i).col(t).t()
//         );
//         if (!std::isfinite(arma::accu(grad))) {
//           grad.fill(std::numeric_limits<double>::max());
//           return std::numeric_limits<double>::max();
...
//       }
//       
//       double val = E_B(t, i, current_s) * log_B1(obs(t, i));
//       if (!std::isfinite(val)) {
//         if (!grad.is_empty()) {
//           grad.fill(std::numeric_limits<double>::max());
//         }
...
//       if (!grad.is_empty()) {
//         grad -= arma::vectorise(
//           E_B(t, i, current_s) * Qt * (I.col(obs(t, i)) - B1) * X_B.slice(i).col(t).t()
//         );
//         if (!std::isfinite(arma::accu(grad))) {
//           grad.fill(std::numeric_limits<double>::max());
//           return std::numeric_limits<double>::max();
...
//   double value = 0;
//   
//   arma::mat eta_Brow = arma::mat(x.memptr(), M(current_c) - 1, K_B);
//   arma::mat gamma_Brow = sum_to_zero(eta_Brow, Qm(current_c));
//   arma::mat Qt = Qm(current_c).t();
//   arma::vec B1(M(current_c));
//   arma::vec log_B1(M(current_c));
//   
//   if (!iv_B && !tv_B) {
//     B1 = softmax(gamma_Brow * X_B.slice(0).col(0));
//     log_B1 = log(B1);
//   }
//   arma::mat I(M(current_c), M(current_c), arma::fill::eye);
//   for (arma::uword i = 0; i < N; i++) {
//     if (iv_B && !tv_B) {
//       B1 = softmax(gamma_Brow * X_B.slice(i).col(0));
//       log_B1 = log(B1);
...
//       }
//       
//       double val = E_B(current_c)(t, i, current_s) * log_B1(obs(current_c, t, i));
//       if (!std::isfinite(val)) {
//         if (!grad.is_empty()) {
//           grad.fill(std::numeric_limits<double>::max());
//         }
...
//       if (!grad.is_empty()) {
//         grad -= arma::vectorise(
//           E_B(current_c)(t, i, current_s) * Qt * (I.col(obs(current_c, t, i)) - B1) * X_B.slice(i).col(t).t()
//         );
//         if (!std::isfinite(arma::accu(grad))) {
//           grad.fill(std::numeric_limits<double>::max());
//           return std::numeric_limits<double>::max();
...
// const bool iv_A, const bool iv_B, const bool tv_A, const bool tv_B, const arma::uword n_obs,
...
//   for (arma::uword c = 0; c < model.C; c++) {
//     previous_pars.cols(
//       n_pi + n_A, n_pi + n_A + arma::accu(n_Bc.rows(0, c - 1)) - 1
