...
void gradient_wrt_omega(
  ...
  tmpmat.diag() += omega;
  grad += Qt * tmpmat * exp(loglik_i - loglik(i)) * X.col(i).t();
}
...
void gradient_wrt_pi(
  ...
  tmpmat = -Pi * Pi.t();
  tmpmat.diag() += Pi;
  grad += Qt * tmpmat * exp(log_py.col(0) + log_beta.col(0) - ll) * X.col(i).t();
}
...
void gradient_wrt_pi(
  ...
  tmpmat = -Pi(d) * Pi(d).t();
  tmpmat.diag() += Pi(d);
  grad += Qt * tmpmat * exp(log_omega(d) + log_py.slice(d).col(0) + 
...
void gradient_wrt_A(
  ...
  grad += Qt * tmpmat * exp(log_alpha(s, t) + log_py.col(t + 1) + 
...
void gradient_wrt_A(
  ...
  grad += Qt * tmpmat * exp(log_omega(d) + log_alpha(s, t, d) + 
...
void gradient_wrt_B_t0(
  ...
  unsigned int idx = obs(0, i);
  double brow = Brow(idx);
  tmpvec = -Brow.t() * brow;
  tmpvec(idx) += brow;
  grad += Qt * exp(log_Pi(s) + log_beta(s, 0) - ll) * tmpvec * X.slice(i).col(0).t();
}
void gradient_wrt_B(
    arma::mat& grad, arma::vec& tmpvec, const arma::mat& Qt,
  ...
  unsigned int idx = obs(t + 1, i);
  double brow = Brow(idx);
  tmpvec = -Brow.t() * brow;
  tmpvec(idx) += brow;
  grad += Qt * arma::accu(exp(log_alpha.col(t) + log_A.slice(t).col(s) + 
...
void gradient_wrt_B_t0(
  ...
  grad += Qt * exp(log_Pi(s) + logpy + log_beta(s, 0) - ll) * tmpvec * 
    X.slice(i).col(0).t();
}
void gradient_wrt_B(
    arma::mat& grad, arma::vec& tmpvec, const arma::mat& Qt,
...
void gradient_wrt_B_t0(
  ...
  tmpvec = -Brow.t() * brow;
  tmpvec(idx) += brow;
  grad += Qt * exp(log_omega(d) + log_Pi(d)(s) + log_beta(s, 0, d) - 
...
void gradient_wrt_B(
  ...
  tmpvec = -Brow.t() * brow;
  tmpvec(idx) += brow;
  grad += Qt * arma::accu(exp(log_omega(d) + log_alpha.slice(d).col(t) + 
...
void gradient_wrt_B_t0(
  ...
  grad += Qt * exp(log_omega(d) + log_Pi(d)(s) + logpy + 
