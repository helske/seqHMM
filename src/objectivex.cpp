...
List objectivex(const arma::mat& transition, NumericVector emissionArray,
                ...
                const arma::mat& coef, const arma::mat& X, arma::ivec& numberOfStates, int threads) {
        ...
        for (int jj = 0; jj < numberOfStates.n_elem; jj++) {
          ...
          for (int i = 0; i < numberOfStates(jj); i++) {
            ...
            if (ind.n_elem > 0) {
              ...
              //   arma::vec tmp = beta.slice(k).col(t + 1).rows(ind_jj,cumsumstate(jj) - 1) / scales(t + 1, k);
              //   
              //   for (unsigned int r = 0; r < obs.n_rows; r++) {
              //     tmp %= emission.slice(r).col(obs(r, t + 1, k)).rows(ind_jj,cumsumstate(jj) - 1);
              //   }
              //   gradArow += alpha(ind_jj + i, t, k) * tmp;
              //   
              // }
