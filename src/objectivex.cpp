...
List objectivex(const arma::mat& transition, NumericVector emissionArray,
    ...
    IntegerVector emissNZ, const arma::ivec& INZ, IntegerVector nSymbols, const arma::mat& coef,
            ...
            for (int j = 0; j < numberOfStates(jj); j++) {
              for (unsigned int t = 0; t < (obs.n_cols - 1); t++) {
                ...
                for (unsigned int r = 0; r < obs.n_rows; r++) {
                  tmp *= emission(cumsumstate(jj) - numberOfStates(jj) + j, obs(r, t + 1, k), r);
                }
      ...
      for (int i = 0; i < numberOfStates.size(); i++) {
        ...
        if (ind.n_elem > 0) {
          ...
          for (int j = 0; j < numberOfStates(i); j++) {
            double tmp = 1.0;
            for (unsigned int r = 0; r < obs.n_rows; r++) {
              tmp *= emission(cumsumstate(i) - numberOfStates(i) + j, obs(r, 0, k), r);
            }
