...
void nhmm_sc::compute_state_obs_probs_fanhmm(
  ...
  for (arma::uword i = 0; i < N; i++) {
    ...
    if (start <= Ti(i)) {
      ...
      log_state_prob_old.col(obs(start, i)) = state_prob.slice(i).col(start);
      state_prob.slice(i).cols(0, start) = arma::exp(state_prob.slice(i).cols(0, start));
      obs_prob.slice(i).cols(0, start) = arma::exp(obs_prob.slice(i).cols(0, start));
      arma::mat tmp(S, M);
      for (arma::uword t = start + 1; t < Ti(i); t++) {
