#' Define Arguments for Plotting Multichannel Aequences and/or Most Probable Paths from Hidden Markov Models
#'
#' Function \code{defineSPS} defines the arguments for later use in \code{\link{SPS}} or \code{\link{SPSgrid}}.
#' 
#'
#'
#' @export
#' 
#' @param x Either hidden Markov model object of class \code{HMModel} or a sequence object created with the \code{\link{seqdef}} function or a list of sequence objects.
#' 
#' @param mpp Output from \code{\link{mostProbablePath}} function. Optional, if \code{x} is a HMModel object or if \code{type=="obs"}.

#' @param plots What to plot (one of "obs" for observations, "mpp" for most probable paths (Viterbi paths), or "both" for observations and most probable paths).

#' @param type The type of the plot. Available types are "I" for index plots and "d" for state distribution plots. See \code{\link{seqplot}} for details.

#' @param sortv A sorting variable or a sort method (one of "from.start", "from.end", or "mds.mpp") for type=="I". The value "mds.mpp" is only available for \code{which="both"} and \code{which="mpp"}. It automatically  arranges the sequences according to the scores of multidimensional scaling (using \code{\link{cmdscale}}) for the hidden states path data from \code{\link{mostProbablePaths}}. See \code{\link{plot.stslist}} for more details on "from.start" and "from.end".

#' @param sort.channel The number of the channel according to whichthe \code{"from.start"} or \code{"from.end"} sorting is done. Sorting according to hidden states is called with value 0. The default value is 1 (the first channel).

#' @param with.missing Controls whether missing states are included in state distribution plots (\code{type="d"}). The default is FALSE.

#' @param title Title for the graphic. The default is NA: if \code{title.n=TRUE}, the number of subjects is still plotted. FALSE prints no title, even when \code{title.n=TRUE}.

#' @param title.n Controls whether the number of subjects is printed in the title of the plot. The default is TRUE: n is plotted if \code{title} if anything but \code{FALSE}.

#' @param cex.title Expansion factor for setting the size of the font for the title. The default value is 1. Values lesser than 1 will reduce the size of the font, values greater than 1 will increase the size.

#' @param title.pos Controls the position of the main title of the plot. The default value is 1. Values grater than 1 will print the title higher.

#' @param withlegend Defines if and where the legend for the states is plotted. The default value \code{"auto"} (equivalent to \code{TRUE} and \code{right.many}) creates separate legends for each requested plot and sets the positions automatically. Other possible values are \code{"right"}, \code{"bottom"} and \code{"bottom.many"}, of which the first two create a combined legend in the selected position and the last one creates separate legends for each requested plot at the bottom of the graph. Value \code{FALSE} prints no legend.

#' @param ncol.legend (A vector of) the number of columns for the legend(s). The default \code{"auto"} creates one column for each legend.

#' @param with.missing.legend If set to \code{"auto"} (the default), a legend for the missing state is added automatically if one or more of the sequences in data contains missing states. With the value \code{TRUE} a legend for the missing state is added in any case; equivalently \code{FALSE} omits the legend for the missing state.

#' @param legend.prop Sets the proportion of the graphic area used for plotting the legend when \code{withlegend} is not \code{FALSE}. The default value is 0.3. Takes values from 0 to 1.

#' @param cex.legend Expansion factor for setting the size of the font for the labels in the legend. The default value is 1. Values lesser than 1 will reduce the size of the font, values greater than 1 will increase the size.

#' @param mpp.color A vector of colors assigned to hidden states. The default value \code{"auto"} uses the colors assigned to the stslist object created with \code{seqdef} if \code{mpp} is given; otherwise gray scale colors are automatically created according to the number of hidden states.

#' @param mpp.labels Labels for the hidden states. The default value \code{"auto"} uses the names provided in \code{x$stateNames} if \code{x} is an HMModel object; otherwise the number of the channel.

#' @param xaxis Controls whether an x-axis is plottted below the plot at the bottom. The default value is \code{TRUE}.

#' @param xlab An optional label for the x-axis. If set to NA, no label is drawn.

#' @param xtlab Optional labels for the x-axis tick labels.  If unspecified, the column names of the \code{seqdata} sequence object are used (see \code{\link{seqdef}}).

#' @param xlab.pos Controls the position of the x axix label. The default value is 2.

#' @param ylab Labels for the channels. A vector of names for each channel (observations). The default value \code{"auto"} uses the names provided in \code{x$channelNames} if \code{x} is an HMModel object; otherwise the number of the channel. \code{FALSE} prints no labels.

#' @param hiddenStates.title Optional label (for the y-axis) for the hidden state plot. The default is \code{"Hidden states"}.

#' @param ylab.pos Controls the position of the y axix labels (labels for channels and/or hidden states). Either \code{"auto"} or a numerical vector indicating on how far away from the plots the titles are positioned. The default value \code{"auto"} positions all titles on line 1.

#' @param cex.lab Expansion factor for setting the size of the font for the axis labels. The default value is 1. Values lesser than 1 will reduce the size of the font, values greater than 1 will increase the size.

#' @param cex.axis Expansion factor for setting the size of the font for the axis. The default value is 1. Values lesser than 1 will reduce the size of the font, values greater than 1 will increase the size.

#' @param ... Other arguments to be passed to \code{\link{seqplot}} to produce the appropriate plot method.
...
#' ssp4 <- defineSPS(HMM$model, type="I", plots="mpp", 
...
#' SPS(ssp4)
#' 
#' @return Object of class \code{defineSPS}.
#' 
#' @seealso \code{\link{SPS}} for plotting objects created with \code{defineSPS}, \code{\link{SPSgrid}} for plotting multiple objects created with \code{defineSPS}, and \code{\link{buildHMM}} and \code{\link{fitHMM}} for building and fitting Hidden Markov models.


defineSPS <- function(x, mpp=NULL,
                         plots="both", type="I", 
  ...
  class(arguments) <- "defineSPS"
  arguments
  
}

# defineSPS(list(child.seq, marr.seq, left.seq), type="d", plots="obs", sortv="from.start", sort.channel=0)
# 
# ssp <- msp.def(list(child.seq, marr.seq, left.seq), type="I", plots="obs", 
#              sortv="from.start", sort.channel=2, 
#              ylab.pos=c(1,2,1), cex.lab=1, ylab=c("Children", "Married", "Left home"), 
#              withlegend=FALSE)
