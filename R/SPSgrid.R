#' Plot Multiseqplots in a Grid
#'
#' Function \code{SPSgrid} plots multiple \code{\link{SPS}}s to a grid.
#' 
#'
#'
#' @export
...
#' @import grid
#' 
#' @param x A list of \code{defineSPS} objects from using \code{\link{SPS}} function.
#' 
#' @param rows,cols Optional arguments to arrange plots.
#' 
#' @param byrow Controls the order of plotting. Defaults to FALSE, i.e. plots are arranged columnwise.
#' 
#' @param withlegend Defines if and how the legends for the states are plotted. The default value \code{"auto"} (equivalent to \code{TRUE} and \code{"many"}) creates separate legends for each requested plot. Other possibilities are \code{"combined"} (all legends combined) and \code{FALSE} (no legend).
#' 
#' @param legend.pos Defines the positions of the legend boxes relative to the whole plot. Either one of \code{"bottom"} (equivalent to \code{"auto"}) or \code{"right"}, or a numerical vector of grid cells (by order) to print the legends to.
#' 
#' @param legend.pos2 Defines the positions of the legend boxes relative to the cell(s). One of"bottomright", "bottom", "bottomleft", "left", "topleft", "top", "topright", "right" and "center".
#'
#' @param title.legend The titles for the legend boxes. The default "auto" takes the titles fromthe channel names provided by the first object in \code{x}. \code{NA} prints no title.
#' 
...
#' 
#' @param row.prop Sets the proportion of the rows of the grid. The default value is \code{"auto"} for even row heights. Takes a vector of values from 0 to 1, with values summing to 1.
#' 
#' @param col.prop Sets the proportion of the columns of the grid. The default value is \code{"auto"} for even column widths. Takes a vector of values from 0 to 1, with values summing to 1.
#' 
#' @examples 
#' require(TraMineR)
#' data(biofam)
...
#' ssp_f <- defineSPS(list(child.seq[biofam$sex=="woman",], 
...
#' ssp_m <- defineSPS(list(child.seq[biofam$sex=="man",], 
...
#' SPSgrid(list(ssp_f, ssp_m), cols=2, withlegend=FALSE)
#' 
#' 
#' # Preparing plots for state distributios and index plots of observations for women
#' ssp_f2 <- defineSPS(list(child.seq[biofam$sex=="woman",], 
#' marr.seq[biofam$sex=="woman",], left.seq[biofam$sex=="woman",]), 
#'                     type="d", plots="obs", title="Women", 
#'                     ylab=c("Children", "Married", "Left home"), withlegend=FALSE)
#' ssp_f3 <- defineSPS(list(child.seq[biofam$sex=="woman",], 
#' marr.seq[biofam$sex=="woman",], left.seq[biofam$sex=="woman",]), 
#'                     type="I", plots="obs", title="Women", 
#'                     ylab=c("Children", "Married", "Left home"), withlegend=FALSE)
#' 
#' # Preparing plots for state distributios and index plots of observations for men
#' ssp_m2 <- defineSPS(list(child.seq[biofam$sex=="man",], 
#' marr.seq[biofam$sex=="man",], left.seq[biofam$sex=="man",]), 
#'                     type="d", plots="obs", title="Men", 
#'                     ylab=c("Children", "Married", "Left home"), withlegend=FALSE)
#' ssp_m3 <- defineSPS(list(child.seq[biofam$sex=="man",], 
#' marr.seq[biofam$sex=="man",], left.seq[biofam$sex=="man",]),
#'                     type="I", plots="obs", title="Men", 
#'                     ylab=c("Children", "Married", "Left home"), withlegend=FALSE)
#' 
#' # Plotting state distributions and index plots of observations for women and men in two columns 
#' SPSgrid(list(ssp_f2, ssp_f3, ssp_m2, ssp_m3), cols=2, byrow=TRUE, 
#'                   withlegend="combined", legend.pos="bottom", row.prop=c(0.4,0.4,0.2))
#'                   
#'                   
#' @seealso \code{\link{defineSPS}} for defining the plot before using \code{SPSgrid}, and \code{\link{SPS}} for plotting only one object created with \code{\link{defineSPS}}.

SPSgrid <- function(x, rows=NA, cols=NA, byrow=FALSE,
  ...
  for(j in 1:ngridplots){
    if(!inherits(x[[j]], "defineSPS")){
      stop("At least one of your objects in x is not a defineSPS object. Use defineSPS to create one.")
    }
  }
