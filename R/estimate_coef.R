...
estimate_coef <- function(model, threads = 1, log_space = FALSE, ...){
  ...
  initNZ <- rep(0, model$n_states)
  transNZ <- matrix(0, model$n_states, model$n_states)
  npCoef <- length(model$coefficients[,-1])
  model$coefficients[,1] <- 0
  initialvalues <- model$coefficients[,-1]
  ...
  objectivef_mhmm <- function(pars, model, estimate = TRUE){
    ...
    model$coefficients[,-1] <- pars

    if (estimate) {
      if (!log_space) {
        objectivex(model$transition_probs, emissionArray, model$initial_probs,
          obsArray, transNZ, emissNZ, initNZ, model$n_symbols,
  ...
  res <- nloptr(x0 = initialvalues, eval_f = objectivef_mhmm,
    opts = list(maxeval = 10000, algorithm = "NLOPT_LD_LBFGS",
      xtol_rel = 1e-8, ftol_rel = 1e-10), model = model, estimate = TRUE, ...)
  model <- objectivef_mhmm(res$solution, model, FALSE)


  if (res$status < 0) {
    warning(paste("Optimization terminated:", res$message))
  }

  original_model$coefficients[] <- model$coefficients

  list(model = original_model, results = res, call = match.call())
}
