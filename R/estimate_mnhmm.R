...
estimate_mnhmm <- function(
    ...
    cluster_names = NULL, inits = "random", init_sd = 2, restarts = 1L, 
    threads = 1L, ...) {
  
  model <- build_mnhmm(
    observations, n_states, n_clusters, initial_formula, 
    transition_formula, emission_formula, cluster_formula, data, data0, 
    state_names, channel_names, cluster_names
    )
  fit_mnhmm(model, inits, init_sd, restarts, threads, ...)
}
