...
test_that("'cluster_names' work for 'mhmm' objects", {
  ...
  expect_equal(cluster_names(mhmm_biofam), 1:3)
})
...
test_that("'cluster_names' work for 'mnhmm' objects", {
  ...
  expect_equal(cluster_names(fit), paste0("Cluster ", 1:2))
...
test_that("'state_names' work for 'hmm' objects", {
  expect_equal(
    state_names(hmm_biofam), paste0("State ", 1:5)
  )
  expect_error(
...
test_that("'state_names' work for 'mhmm' objects", {
  expect_equal(
    ...
    list(
      `Cluster 1` = c("State 1", "State 2", "State 3", "State 4"), 
      `Cluster 2` = c("State 1", "State 2", "State 3", "State 4"), 
      `Cluster 3` = c("State 1", "State 2", "State 3", "State 4", 
                      "State 5", "State 6")
    )
  )
...
test_that("'most_probable_cluster' works", {
  ...
  expect_equal(
    head(most_probable_cluster(mhmm_biofam), 4),
    paste0("Cluster ", c(3, 2, 3, 3))
  )
  expect_equal(
    head(most_probable_cluster(mhmm_biofam, type = "posterior"), 4),
    paste0("Cluster ", c(3, 2, 3, 3))
  )
})
